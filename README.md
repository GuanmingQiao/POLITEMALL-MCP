...
1. Go to **[/connect](https://13-212-182-50.sslip.io/connect)** and click
   **Generate a new token**. It's shown once — save it into your MCP client's
   config immediately. Lost it? Generate a new one; there's no recovery.
2. Connect your session cookie(s): log in normally in your own browser — on the
   polytechnic corporate network/VPN, since that's the only place SSO works — at
   whichever of `lms.polite.edu.sg`, `nyplms.polite.edu.sg`, `stms.polite.edu.sg`
   you want, then either:
   - **[Install the cookie sync extension](extension/README.md)** (recommended) —
     one click instead of DevTools copy-paste, or
   - On `/connect`: open DevTools → Network tab, copy the `Cookie` request header
     for a request to that domain, and paste it into the matching field.
