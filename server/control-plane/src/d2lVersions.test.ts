...
import { getD2lVersions, compareVersions, pickLatest, resetD2lVersionCache, D2lVersionDiscoveryError } from "./d2lVersions.js";
...
test("apiGet requests use the discovered version for le and lp routes", async (t) => {
  ...
  await d2l.callD2lOperation("nyp", "cookie", "le.grades.finalValuesAll", {}, {}, 524042);
  await d2l.whoami("nyp", "cookie").catch(() => undefined);
...
test("D2L HTTP failures keep their status, route and D2L's own detail", async (t) => {
  ...
  await assert.rejects(
    () => d2l.callD2lOperation("nyp", "cookie", "le.grades.finalValuesAll", {}, {}, 524042),
    (err: unknown) =>
      err instanceof d2l.BadRequestError &&
      err.status === 400 &&
...
test("getCourseAccess reports role for an enrolled course and enrolled:false for a 404", async (t) => {
  ...
  assert.deepEqual(await d2l.getCourseAccess("nyp", "cookie", 524042), { enrolled: true, role: "Tutor", isActive: true });
  status = 404;
  assert.deepEqual(await d2l.getCourseAccess("nyp", "cookie", 1), { enrolled: false, role: null, isActive: null });
});
