...
// spec: d2l-mcp-tool-surface — "generic tool invokes a cataloged operation", "generic tool
...
test("callD2lOperation: a caller cannot override orgUnitId by smuggling it into pathParams", async (t) => {
  ...
  // caller-supplied value even before the trusted orgUnitId argument is applied — the request
...
test("callD2lOperation: objectList pagination is followed across pages", async (t) => {
  ...
  t.mock.method(globalThis, "fetch", async (url: string) => {
    ...
    if (seen.length === 1) return jsonResponse({ Next: "/d2l/api/le/1.9/6606/quizzes/?page=2", Objects: [{ QuizId: 1 }] });
