...
*existing* token with a fresh cookie — your MCP client's config never needs to
change, since it only ever holds the token, not the cookie. Tokens themselves expire
after 90 days regardless of use; when that happens you do need a new token (and to
update your MCP client's config with it).
